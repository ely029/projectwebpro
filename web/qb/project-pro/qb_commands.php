...
function _quickbooks_item_query_response($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents)
{
	...
	foreach ($purchases as $purchase) {
		...
		if (count($purchaseItemsFiltered) >= $countPurchaseItems) { // If Purchase Items Exist and are Validated proceed to Add Bill
			...
			} else if ($transactionType == 'DEBIT/BANK') {
				if(!$isTotalAmountNegative){
					$Queue->enqueue(QUICKBOOKS_ADD_CHECK, 'purchase_' . $purchase['key'], 1, $purchaseItemsFiltered, $user, null, true);
				} else {
					$Queue->enqueue(QUICKBOOKS_ADD_CHECK, 'purchase_' . $purchase['key'], 1, $purchaseItemsFiltered, $user, null, true);
				}
...
function validate_vendor($availableVendors, &$purchaseItems)
{
	...
	foreach ($purchaseItems as &$pi) {
		...
		if ($pi['vendor'] == null) {
			$errorMessage = ('Vendor not found. The Web Connector could not find the vendor in ProjectPro. Go to ProjectPro > Administration > Vendors. Make sure the spelling is identical in ProjectPro and QuickBooks.');
		...
		if (in_array(strtoupper($pi['vendor']), $availableVendors)) {
			//$result[] = $pi;
		} else if (in_array(strtoupper(str_replace('’', '\'', $pi['vendor'])), $availableVendors)){
			$pi['vendor'] = str_replace('’', '\'', $pi['vendor']);
		}  else if (in_array(strtoupper(str_replace('\'', '’', $pi['vendor'])), $availableVendors)){
			$pi['vendor'] = str_replace('\'', '’', $pi['vendor']);
		} else {
			//$fp = fopen('log.txt', 'w');
			...
			$errorMessage = ('Vendor ' . strtoupper($pi['vendor']) . ' not found. The Web Connector could not find the vendor in QB. Go to ProjectPro > Administration > Vendors. Make sure the spelling is identical in ProjectPro and QuickBooks.');
			report_exception($pi['id'], 2, $errorMessage);
			$pi['is_exception'] = true;
			continue;
		}
...
function validate_class($availableClasses, &$purchaseItems)
{
	foreach ($purchaseItems as &$pi) {
		...
		// else if (in_array(strtoupper(str_replace('’', '\'', $pi['purchase_class'])), $availableClasses)){
		// 	$pi['purchase_class'] = str_replace('’', '\'', $pi['purchase_class']);
		// } 
		// else if (in_array(strtoupper(str_replace('\'', '’', $pi['purchase_class'])), $availableClasses)){
		// 	$pi['purchase_class'] = str_replace('\'', '’', $pi['purchase_class']);
		// } 
		else {
			$errorMessage = ('Class ' . strtoupper($pi['purchase_class']) . ' not found. The Web Connector could not find the class in QuickBooks. Either add the class to QuickBooks or remove the class from ProjectPro');
...
function validate_customer($availableCustomers, &$purchaseItems)
{
	...
	foreach ($purchaseItems as &$pi) {
		if (in_array(strtoupper($pi['project']), $availableCustomers)) {
			//$result[] = $pi;
		} else if (in_array(strtoupper(str_replace('’', '\'', $pi['project'])), $availableCustomers)){
			$pi['project'] = str_replace('’', '\'', $pi['project']);
		} else if (in_array(strtoupper(str_replace('\'', '’', $pi['project'])), $availableCustomers)){
			$pi['project'] = str_replace('\'', '’', $pi['project']);
		}  else {
...
function validate_cost_items($availableCostCodes, $listAccounts, &$purchaseItems)
{
	...
	foreach ($purchaseItems as &$pi) {
		...
		$fullNameCostCode = trim(strtoupper($pi['full_cost_code']));
		if (array_search($costCode, array_column($availableCostCodes, 'name')) !== false) {
			// continue
		} else if (array_search($fullNameCostCode, array_column($availableCostCodes, 'name')) !== false) {
			// continue
			$pi['cost_code'] = $pi['full_cost_code'];
		} else {
			//fwrite($fp, print_r($costCode, true));
			$errorMessage = ('Cost Code Item ' . $costCode . ' not found. The Web Connector could not find the cost code in QuickBooks. Go to ProjectPro > Administration > Projects > Edit/View the project listed. Delete or disable the cost code from the project so it does not get used in future purchases.');
...
function get_expense_account(&$pi, $availableAccounts)
{
	...
	$errorMessage = ('Expense Account ' . strtoupper($pi['cost_code']) . ' not found in Quickbooks. The Web Connector could not find the expense account in QuickBooks. Go to ProjectPro > Administration > Projects > Edit/View the project listed. Delete or disable the cost code from the project so it does not get used in future purchases, or add ' . strtoupper($pi['cost_code']) . ' to QuickBooks Chart of Accounts.');
	report_exception($pi['id'], 3, $errorMessage);
	$pi['is_exception'] = true;
	return null;
}
...
function validate_paymenttypes($availableAccounts, &$purchaseItems, $transactionType)
{
	...
	if ($transactionType != 'CREDIT CARD'  && $transactionType != 'DEBIT/BANK') {
		return;
	...
	foreach ($purchaseItems as &$pi) {
		...
		if (array_search(strtoupper($pi['payment_type']), array_column($availableAccounts, 'Name')) !== false) {
			...
			assign_account_fullname($pi, $availableAccounts);
		} else if (strpos($pi['payment_type'], '\'') !== false) {
			$pi['payment_type'] = str_replace('\'', '’', $pi['payment_type']);
			if (array_search(strtoupper($pi['payment_type']), array_column($availableAccounts, 'Name')) !== false) {
				assign_account_fullname($pi, $availableAccounts);
			}
		} else if (strpos($pi['payment_type'], '’') !== false) {
			...
			$errorMessage = ('Credit card or Debit account not found. The Web Connector could not find payment type ' . strtoupper($pi['payment_type']) . ' in QuickBooks. Go to your QuickBooks Chart of Accounts and either add the payment type as a credit card/check or correct the account name to match the one in ProjectPro.');
...
function validate_accounts($availableAccounts, &$purchaseItems, $transactionType)
{
	...
	foreach ($purchaseItems as &$pi) {
		if (in_array($pi['payment_type'], $availableAccounts)) {
			...
			$errorMessage = ('Payment Type ' . strtoupper($pi['payment_type']) . ' not found. The Web Connector could not find the credit card in QuickBooks. Either add the credit card to the chart of accounts in QuickBooks, correct the account name in QuickBooks or go to ProjectPro - Administration - Users > View/Edit each user > remove the listed payment type & add the payment type with the same spelling as listed in QuickBooks.');
			report_exception($pi['id'], 6, $errorMessage);
			$pi['is_exception'] = true;
		}
	}
...
function _quickbooks_bill_add_request($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale)
{
	...
	$purchaseId = '2019-' . $items[0]['purchase_id'];
...
function _quickbooks_check_add_request($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale)
{
	$items = $extra;
