...
//            return new Response('No sales tax', 500);
        ...
        $this->notifyOverBudget($costs, $tmpAmount, $employeeName, $purchaser, $totalAmount, $projectName);
    ...
    private function notifyOverBudget($costs, $amount, $employeeName, $purchaser, $totalAmount, $projectName) {
        $overBudgetCosts = [];
        ...
        if(count($overBudgetCosts) > 0){
            ...
            <li>Open project ".$projectName."</li>
...
//    }
    ...
    /**
     * @Route("/transaction/{id}", name="apiTransactions")
     * @Method("GET")
     */
    public function transactionAction($id,Request $request) {
          ...
          $statement = $em->getConnection()->prepare('select id as transactionId,
              ...
              from transactions where user_id =:a and is_deleted <> 1 and status <> :b');
     ...
     * @Method("POST")
     */
    public function transactionPostAction(Request $request) {



            $image = $request->request->get('imageUrl');
            $amount = $request->request->get('amount');
            $type = $request->request->get('type');

            if(empty($image)){

                return new JsonResponse(['error' => 'true','message' => 'Image url is missing']);
            } else {
