...
class UserHandler
{
     ...
     */
    public function createUser(User $user, User $admin, $roles = [], $isDefaultAccountant = false) {
        $employee = $this->entityManager->getRepository('AppBundle:Employee')->findOneBy(['user' => $user]);
        ...
        if ($employee) {
            ...
            $employee = $this->entityManager->getRepository('AppBundle:Employee')
                ->createUser($user, $roles, $isDefaultAccountant);
